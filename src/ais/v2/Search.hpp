...
namespace AI_V2 {
    ...
    inline int score_move_for_ordering(const GameState& state, const Move& move, int player_id) {
        // On simule le coup rapidement (copie locale)
        // Note : C'est coûteux de copier l'état, mais ça fait gagner énormément de temps de recherche.
        GameState sim_state = state;
        GameRules::apply_move(sim_state, move, player_id);

        int score_gain = (player_id == 1)
            ? (sim_state.score_p1 - state.score_p1)
            : (sim_state.score_p2 - state.score_p2);

        // Si le coup rapporte des points, c'est très bien (+1000 par point)
        // Sinon, on peut ajouter d'autres critères (ex: capturer des graines)
        return score_gain;
    }
    ...
    inline Move find_best_move(const GameState& root_state, int player_id, double time_limit_sec) {
        ...
        std::cout << "[AI v2] D:" << stats.max_depth
            << " Nodes:" << stats.nodes
