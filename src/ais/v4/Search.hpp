...
namespace AI_V4 {
    ...
    // --- HEURISTIQUE AMÉLIORÉE : GESTION DES PRISES MULTIPLES ---
    inline int score_move_for_ordering(const GameState& state, const Move& move, int player_id) {
        ...
        // Score de base = nombre de graines semées (activité)
        int heuristic_score = seeds;

        // Calcul de l'arrivée
        int final_hole = (move.hole + seeds) % NB_HOLES;

        // --- BOUCLE DE RETOUR POUR LES PRISES MULTIPLES ---
        // On regarde la case d'arrivée, puis celle d'avant, puis celle d'avant...
        // Comme dans la vraie règle de prise.

        int current_check_hole = final_hole;
        bool capturing = true;

        // On limite la vérification à quelques cases en arrière pour rester rapide (ex: 3 ou 4 max)
        // C'est une approximation, mais bien plus précise.
        int check_limit = 0;

        while (capturing && check_limit < 4) {
            // Si on est revenu dans notre camp, on arrête les prises (selon les règles standards)
            if (GameRules::is_current_player_hole(current_check_hole, player_id)) {
                break;
            }

            // On compte les graines. 
            // ATTENTION: Pour la case finale exacte, on ajoute notre graine (+1).
            // Pour les cases précédentes, on regarde juste ce qu'il y a.
            int existing = state.get_seeds(current_check_hole, RED)
                + state.get_seeds(current_check_hole, BLUE)
                + state.get_seeds(current_check_hole, TRANSPARENT);

            int total_seeds = existing;
            if (current_check_hole == final_hole) total_seeds++; // La graine qu'on vient de poser

            if (total_seeds == 2 || total_seeds == 3) {
                heuristic_score += total_seeds * 100; // Jackpot !

                // On recule d'une case (modulo circulaire inversé)
    ...
    inline Move find_best_move(const GameState& root_state, int player_id, double time_limit_sec) {
        ...
        for (int depth = 1; depth <= 64; ++depth) {
            ...
            for (const auto& move : moves) {
                ...
                // NOUVEAU : Si le score est égal au meilleur, on a une chance de changer
                // Cela casse les boucles infinies de coups identiques
                else if (score == best_score_this_depth) {
                    // 50% de chance de changer pour ce coup équivalent
                    if (rand() % 2 == 0) {
                        best_move_this_depth = move;
                    }
                }
        ...
        std::cout << "[AI v4] D:" << stats.max_depth
            << " Nodes:" << stats.nodes
