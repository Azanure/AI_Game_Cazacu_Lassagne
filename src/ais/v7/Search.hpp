...
namespace AI_V7 {
    ...
    struct BotDNA {
        ...
        int w_seed_blue = 192;
        int w_seed_trans = 2;
        int w_defense = -68;       // Stratégie de baiting
        int w_attack = 28;        // Agressivité calibrée
        int w_mobility = 81;      // Contrôle de terrain massif
        int w_hoard_penalty = 44; // Propreté du plateau
        int w_starvation = -13;   // Gestion de fin de partie tactique

        // Poids positionnels optimisés
        std::array<int, 16> w_holes = { 1, 12, 25, 9, -8, 4, 1, -3, 3, 11, -9, -2, 4, 12, 2, 8 };
    };
